...
class TextAnalysis:
    ...
    async def analyze_text(client, text):
        ...
        async with client:
            ...
            key_phrases_result = await client.extract_key_phrases([text])
        ...
        return {
            ...
            "key_phrases": key_phrases_result[0] if key_phrases_result else None,
    ...
    async def save_results(filename, results):
        ...
        Args:
            filename (str): The filename or identifier associated with the text.
        ...
        with open(json_filename, 'w') as json_file:
            json.dump(results, json_file, default=lambda x: x.__dict__, indent=4)
        ...
        with sqlite3.connect(db_filename) as conn:
            ...
            cursor.execute(
                ...
                f"VALUES (?, ?)",
                (filename, json.dumps(results, default=lambda x: x.__dict__)),
            )
            conn.commit()
...
async def analyze_text_files(file_path):
    ...
    Args:
        file_path (str): Path to the folder containing text files to be processed.
    """
    try:
        ...
        for filename in os.listdir(config.transcripts):
            if filename.endswith('.txt'):
                ...
                with open(input_file, 'r') as file:
                    text = file.read()
                await TextAnalysis.process_text(client, text, input_file)
...
if __name__ == '__main__':
    ...
    asyncio.run(analyze_text_files())
