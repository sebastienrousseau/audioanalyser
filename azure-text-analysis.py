...
async def save_results(filename, results):
    ...
    with open(txt_filename, 'w') as file:
        ...
        if entities_result.entities:
            file.write('   • Key Entities Identified: ' + ', '.join([entity.text for entity in entities_result.entities[:3]]) + '.\n')

        # Key Phrases Summary
        key_phrases_result = results['key_phrases'][0]
        if key_phrases_result.key_phrases:
            file.write('   • Notable Topics: ' + ', '.join(key_phrases_result.key_phrases[:3]) + '.\n')
    ...
    # Save to TXT
    txt_filename = f"{filename}_analysis.txt"
    with open(txt_filename, 'w') as file:
        file.write(json.dumps(serializable_results, indent=4, ensure_ascii=False))
    print(f"Saving results to {txt_filename}")

    # Save to JSON
    json_filename = f"{filename}_analysis.json"
    with open(json_filename, 'w') as json_file:
        json.dump(serializable_results, json_file, indent=4)
    print(f"Saving results to {json_filename}")

    # Save to SQLite
    db_filename = 'text_analysis.db'
    with sqlite3.connect(db_filename) as conn:
        cursor = conn.cursor()
        cursor.execute(f'''CREATE TABLE IF NOT EXISTS {DB_TABLE_NAME}
                          (filename TEXT, analysis TEXT)''')
        cursor.execute(f"INSERT INTO {DB_TABLE_NAME} (filename, analysis) VALUES (?, ?)",
                       (filename, json.dumps(serializable_results)))
        conn.commit()
    print(f"Saved analysis of {filename} to database.")
